...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        pie_data = spacex_df.groupby('Launch Site').sum().reset_index()
        fig = px.pie(pie_data, values='class', 
        names= pie_data["Launch Site"], 
        title='Total Success Launch By Site')
        return fig
    else:
        # return the outcomes piechart for a selected site
        # filtered_df
        fig = px.pie( values=filtered_df["class"].value_counts().values,
                      names= filtered_df["class"].value_counts().index ,
                      title='Success Launch By {}'.format(entered_site))
        return fig
...
def get_scatter_chart(entered_site , payload_range ) :  
    p_range = spacex_df[ spacex_df["Payload Mass (kg)"].between(payload_range[0],payload_range[1]) ]
    if entered_site == 'ALL':
        fig = px.scatter(p_range, x="Payload Mass (kg)", y="class" , color = "Booster Version Category")
        return fig
    else :
        filtered_df = p_range[ p_range["Launch Site"] == entered_site ]
